...
- Node IDs use `node_` followed by a full UUIDv7 generated with cryptographic randomness. The OS hostname is captured at initial creation and remains stable with the identity. The local node's trust state is `trusted`; peer trust is stored separately in migration 5 and checked against stable Tailscale device bindings. See [trust](trust.md).
