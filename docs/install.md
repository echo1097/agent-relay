...
Run as the same login user with the same `HOME` used for installation. If the installation used `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `XDG_CONFIG_HOME`, or `XDG_DATA_HOME`, set the same values when running `update` so client and service locations remain consistent.
...
The running executable invokes the installer bundled in the binary and needs no release download, Tailscale connection, or build tools. It reads the sibling `.agent-relay-receipt`, uses its directory as the executable directory, verifies the executable and receipt match an installer-managed installation, and reuses the recorded Relay home and service name. Missing, malformed, or mismatched receipts are refused. Run as the same login user with the same `HOME`, `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `XDG_CONFIG_HOME`, and `XDG_DATA_HOME` values used during installation so cleanup checks the original client and service locations.
